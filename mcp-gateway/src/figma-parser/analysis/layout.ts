...
export class LayoutAnalyzer {
  ...
  private analyzeAsGrid(node: FigmaNode): GridLayout {
    ...
    const xPositions = Array.from(new Set(children.map(c => Math.round(c.absoluteBoundingBox!.x)))).sort((a, b) => a - b);
    const yPositions = Array.from(new Set(children.map(c => Math.round(c.absoluteBoundingBox!.y)))).sort((a, b) => a - b);
    ...
    if (xPositions.length >= 2) {
      ...
      const secondX = xPositions[1] ?? 0;
      columnGap = Math.max(0, secondX - firstX - avgWidth);
    }

    if (yPositions.length >= 2) {
      const heights = children.map(c => c.absoluteBoundingBox!.height);
      const avgHeight = heights.reduce((a, b) => a + b, 0) / heights.length;
      const firstY = yPositions[0] ?? 0;
      const secondY = yPositions[1] ?? 0;
      rowGap = Math.max(0, secondY - firstY - avgHeight);
    }
  ...
  private getParentBounds(_node: FigmaNode): Rectangle | undefined {
  ...
  private analyzeChildLayout(child: FigmaNode, _parent: FigmaNode): ChildLayoutInfo {
    const sizing = this.analyzeSizing(child);
  ...
  private analyzeSizing(node: FigmaNode): ChildLayoutInfo['sizing'] {
