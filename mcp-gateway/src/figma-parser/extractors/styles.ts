...
export class StyleExtractor {
  ...
  private calculateGradientAngle(handles?: { x: number; y: number }[]): number {
    ...
    return Math.round(angle + 90); // CSS angles are rotated 90deg
  }
  ...
  private extractNamedStyle(styleId: string, _type: string, node: FigmaNode): void {
    const metadata = this.styleMetadata[styleId];
    if (!metadata) return;

    // Check if already extracted
    const existingIndex = this.namedStyles.findIndex(s => s.id === styleId);
    if (existingIndex >= 0) {
      // Increment usage count
      const existing = this.namedStyles[existingIndex];
      if (existing) {
        existing.usageCount++;
      }
      return;
