...
export class ReactGenerator {
  ...
  async generate(design: ParsedDesign): Promise<GenerationResult> {
    ...
    // Input validation
    if (!design) {
      throw new Error('Design input required');
    }
    if (!design.pages) {
      throw new Error('Design must have pages');
    }
  ...
  private buildComponentCode(
    ...
    const _ext = this.config.typescript ? 'tsx' : 'jsx';
    void _ext; // Extension available for future use
...
${name}.displayName = '${name}';`;
  ...
  private generateStyleFile(name: string, styles: any): { styleCode?: string; styleFileName?: string } {
    switch (this.config.stylingApproach) {
      ...
      case 'vanilla':
        return {
          styleCode: this.styleGenerator.generateVanillaCSS(styles, `.${this.nameUtils.toCSSClassName(name)}`),
...
`;
  ...
  private generateTest(name: string, _props: ComponentProp[]): string {
