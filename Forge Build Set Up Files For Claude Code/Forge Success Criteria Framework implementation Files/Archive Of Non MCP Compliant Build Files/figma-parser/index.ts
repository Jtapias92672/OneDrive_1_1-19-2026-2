...
import { FigmaClient, FigmaClientConfig } from './client/figma-client';
import { ComponentExtractor } from './extractors/components';
import { StyleExtractor } from './extractors/styles';
import { LayoutAnalyzer } from './analysis/layout';
import { SemanticAnalyzer } from './analysis/semantic';
import { TokenGenerator } from './tokens/generator';
import {
  ParsedDesign,
  ParsedPage,
  ParsedFrame,
...
export class FigmaParser {
  ...
  parseFileData(file: FigmaFile, fileKey: string = 'local', startTime: number = Date.now()): ParsedDesign {
    ...
    const parsedComponents = this.buildParsedComponents(componentResult.components, file);
  ...
  private buildParsedComponents(components: any[], file: FigmaFile): ParsedComponent[] {
    return components.map(comp => ({
      ...
      props: comp.properties.map((p: any) => ({
        name: p.name,
  ...
  private buildComponentSets(sets: any[]): ParsedComponentSet[] {
    return sets.map(set => ({
      id: set.id,
      name: set.name,
      description: set.description,
      variantIds: set.variants.map((v: any) => v.id),
...
export { FigmaClient, createFigmaClient, getDefaultClient } from './client/figma-client';
export { ComponentExtractor, findComponents, findInstances } from './extractors/components';
export { StyleExtractor } from './extractors/styles';
export { LayoutAnalyzer } from './analysis/layout';
export { SemanticAnalyzer } from './analysis/semantic';
export { TokenGenerator } from './tokens/generator';
export { validateParsedDesign, PARSED_DESIGN_SCHEMA } from './output/schema';
