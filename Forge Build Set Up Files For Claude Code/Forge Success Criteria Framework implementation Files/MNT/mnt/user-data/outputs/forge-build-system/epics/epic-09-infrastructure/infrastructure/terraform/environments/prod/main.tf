...
# -----------------------------------------------------------------------------
# MCP Server IAM (Per-server least-privilege IRSA)
# -----------------------------------------------------------------------------

module "mcp_iam" {
  source = "../../modules/mcp-iam"
  ...
  artifacts_bucket_arn = module.s3.artifacts_bucket_arn
  kms_key_arns         = [module.s3.kms_key_arn]
  bedrock_model_arns   = module.bedrock.allowed_model_arns

  enable_slack_mcp      = true
  enable_database_mcp   = true
  enable_web_search_mcp = true
  enable_bedrock_mcp    = true
...
# -----------------------------------------------------------------------------
# MCP Monitoring (Per-server dashboards and alarms)
# -----------------------------------------------------------------------------

module "mcp_monitoring" {
  source = "../../modules/mcp-monitoring"

  name        = local.name
  environment = local.environment
  aws_region  = var.aws_region

  mcp_servers = ["filesystem", "github", "slack", "database", "web-search"]
...
output "mcp_config" {
  ...
  value = {
    role_arns = module.mcp_iam.mcp_role_arns
    log_group = module.mcp_iam.mcp_log_group_name
    dashboard = module.mcp_monitoring.dashboard_name
  }
}
