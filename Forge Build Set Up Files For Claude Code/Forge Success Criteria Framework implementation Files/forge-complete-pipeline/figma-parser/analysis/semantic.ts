...
const DETECTION_PATTERNS: DetectionPattern[] = [
  // Buttons
  {
    ...
    structureCheck: (node) => {
      // Button-like: has text, has fill, has rounded corners, relatively small
  ...
  {
    type: 'input',
    namePatterns: [/input/i, /field/i, /textfield/i, /text-field/i, /text_field/i, /search/i],
    structureCheck: (node) => {
  ...
  {
    type: 'textarea',
    namePatterns: [/textarea/i, /text-area/i, /multiline/i, /comment/i, /message/i],
    structureCheck: (node) => {
      const bbox = node.absoluteBoundingBox;
      return bbox ? bbox.height > 80 : false;
    },
    baseConfidence: 0.7,
  ...
  {
    type: 'select',
    namePatterns: [/select/i, /dropdown/i, /picker/i, /combobox/i],
    baseConfidence: 0.75,
    htmlElement: 'select',
    ariaRole: 'combobox',
    interactive: true,
  ...
  {
    type: 'checkbox',
    namePatterns: [/checkbox/i, /check-box/i, /check_box/i, /checked/i],
    structureCheck: (node) => {
      const bbox = node.absoluteBoundingBox;
      return bbox ? Math.abs(bbox.width - bbox.height) < 10 && bbox.width < 40 : false;
    },
    baseConfidence: 0.8,
    htmlElement: 'input',
    ariaRole: 'checkbox',
  ...
  // Radio
  {
    ...
    structureCheck: (node) => {
      const bbox = node.absoluteBoundingBox;
      const isCircular = node.cornerRadius && node.cornerRadius > 10;
      return Boolean(bbox && Math.abs(bbox.width - bbox.height) < 5 && bbox.width < 30 && isCircular);
    },
    baseConfidence: 0.75,
    htmlElement: 'input',
    ariaRole: 'radio',
  ...
  {
    type: 'switch',
    namePatterns: [/switch/i, /toggle/i, /on-off/i],
    baseConfidence: 0.8,
    htmlElement: 'input',
    ariaRole: 'switch',
    interactive: true,
  ...
  // List
  {
    ...
    structureCheck: (node) => {
      ...
      // Check if children have similar heights (list-like)
      ...
      const variance = heights.every(h => Math.abs(h - avgHeight) < avgHeight * 0.3);
      return variance;
    },
    baseConfidence: 0.7,
    htmlElement: 'ul',
  ...
  {
    type: 'list-item',
    namePatterns: [/item/i, /row/i, /list-item/i, /listitem/i],
    baseConfidence: 0.6,
    htmlElement: 'li',
    ariaRole: 'listitem',
    interactive: false,
  ...
  {
    type: 'table',
    namePatterns: [/table/i, /grid/i, /data-table/i],
    structureCheck: (node) => {
      // Tables usually have many children arranged in rows
      if (!hasChildren(node) || node.children!.length < 3) return false;
      const childCount = node.children!.length;
      return childCount >= 4; // At least header + a few rows
    },
    baseConfidence: 0.7,
    htmlElement: 'table',
    ariaRole: 'table',
  ...
  {
    type: 'navbar',
    namePatterns: [/navbar/i, /nav-bar/i, /topbar/i, /top-bar/i, /header-nav/i],
    baseConfidence: 0.8,
    htmlElement: 'nav',
    ariaRole: 'navigation',
    interactive: false,
  ...
  {
    type: 'sidebar',
    namePatterns: [/sidebar/i, /side-bar/i, /sidenav/i, /side-nav/i],
    structureCheck: (node) => {
      const bbox = node.absoluteBoundingBox;
      return bbox ? bbox.height > bbox.width * 2 : false;
    },
    baseConfidence: 0.75,
    htmlElement: 'aside',
  ...
  {
    type: 'modal',
    namePatterns: [/modal/i, /dialog/i, /popup/i, /overlay/i, /lightbox/i],
    baseConfidence: 0.8,
    htmlElement: 'dialog',
    ariaRole: 'dialog',
    interactive: true,
  ...
  {
    type: 'form',
    namePatterns: [/form/i, /signup/i, /sign-up/i, /login/i, /register/i],
    baseConfidence: 0.75,
    htmlElement: 'form',
    ariaRole: 'form',
    interactive: true,
  ...
  {
    type: 'image',
    namePatterns: [/image/i, /img/i, /photo/i, /picture/i, /illustration/i, /graphic/i],
    structureCheck: (node) => {
  ...
  {
    type: 'avatar',
    namePatterns: [/avatar/i, /profile-pic/i, /user-image/i, /profile-image/i],
    structureCheck: (node) => {
      const bbox = node.absoluteBoundingBox;
  ...
  {
    type: 'tag',
    namePatterns: [/tag/i, /chip/i, /pill/i, /label/i],
    structureCheck: (node) => {
      const bbox = node.absoluteBoundingBox;
      const hasRadius = (node.cornerRadius || 0) > 8;
      const isSmall = bbox ? bbox.height < 40 && bbox.width < 150 : false;
      return Boolean(hasRadius && isSmall);
    },
    baseConfidence: 0.7,
    htmlElement: 'span',
    interactive: false,
  ...
  {
    type: 'spinner',
    namePatterns: [/spinner/i, /loader/i, /loading/i, /spin/i],
    baseConfidence: 0.75,
    htmlElement: 'div',
    ariaRole: 'progressbar',
    interactive: false,
